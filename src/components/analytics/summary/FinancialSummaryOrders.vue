<template>
  <Card>
    <CardHeader>
      <CardTitle class="text-lg font-medium text-gray-700">Финансовая сводка — Заказы</CardTitle>
    ...
    <CardContent>
      <div class="flex justify-normal gap-4 mb-4">
        <div>
          <p class="text-xs text-muted-foreground">Всего заказов (кол-во)</p>
...
const {getFinancialSummaryOrders, sending, progress} = useAnalyticsFunctions()
...
const fetch = async () => {
  ...
  const res = await getFinancialSummaryOrders(params)
...
function buildChart() {
  ...
  chartData.value = {
    ...
    datasets: [{label: 'Сумма', data: data.value.series.map((s: any) => s.sum)}]
  }
}
</script>
