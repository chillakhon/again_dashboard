                  ...
                  <ul role="list" class="flex flex-1 flex-col gap-y-7">
                      ...
                      <ul role="list" class="-mx-2 space-y-1">
                        <li v-for="item in updatedNavigation" :key="item.name">
                          <router-link v-if="!item.children" :to="item.href"
                                       :class="[item.current ? 'bg-gray-50 text-red-600' : 'text-gray-700 hover:bg-gray-50 hover:text-red-600', 'group flex gap-x-3 rounded-md p-2 text-sm/6 font-semibold']"
                                       @click="closeSidebar">
                            <component :is="item.icon"
                                       :class="[item.current ? 'text-red-600' : 'text-gray-400 group-hover:text-red-600', 'size-6 shrink-0']"
                                       aria-hidden="true"/>
                            {{ item.name }}
                          </router-link>
                          <Disclosure as="div" v-else v-slot="{ open }">
                            <DisclosureButton
                                :class="[item.current ? 'bg-gray-50' : 'hover:bg-gray-50', 'flex w-full items-center gap-x-3 rounded-md p-2 text-left text-sm/6 font-semibold text-gray-700']">
                            ...
                            <DisclosurePanel as="ul" class="mt-1 px-2">
                              <li v-for="subItem in item.children" :key="subItem.name">
                                <router-link :to="subItem.href"
                                             @click="closeSidebar"
                                             :class="[subItem.current ? 'bg-gray-50 text-red-600' : 'text-gray-500 hover:bg-gray-50 hover:text-red-600', 'group flex gap-x-3 rounded-md p-2 text-xs/6 font-semibold']">
                                  <component :is="subItem.icon"
                                             :class="[subItem.current ? 'text-red-600' : 'text-gray-400 group-hover:text-red-600', 'size-5 shrink-0']"
                                             aria-hidden="true"/>
                                  {{ subItem.name }}
                                </router-link>
                              </li>
                    ...
                    <li class="mt-auto">
                      <router-link to="/settings"
                                   class="group -mx-2 flex gap-x-3 rounded-md p-2 text-sm/6 font-semibold text-gray-700 hover:bg-gray-50 hover:text-red-600"
                                   @click="closeSidebar">
