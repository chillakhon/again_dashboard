<template>
  <div class="space-y-6 p-6 max-w-4xl mx-auto">
    ...
    <Card>
      <CardHeader>
        <div class="flex items-center space-x-3">
          <div class="w-10 h-10 rounded-md bg-gradient-to-br from-blue-600 to-blue-800 flex items-center justify-center text-white">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
              <path d="M20 8h-3V4H3c-1.1 0-2 .9-2 2v11h2c0 1.7 1.3 3 3 3s3-1.3 3-3h6c0 1.7 1.3 3 3 3s3-1.3 3-3h2v-5l-3-4zM6 18.5c-.8 0-1.5-.7-1.5-1.5s.7-1.5 1.5-1.5 1.5.7 1.5 1.5-.7 1.5-1.5 1.5zm9-3.5H8.2c-.5-.6-1.3-1-2.2-1H3V6h12v9zm3 3.5c-.8 0-1.5-.7-1.5-1.5s.7-1.5 1.5-1.5 1.5.7 1.5 1.5-.7 1.5-1.5 1.5zM17 12V8h2l2 2.5L17 12z"/>
            </svg>
          </div>
      ...
      <CardContent class="space-y-4" :class="{'opacity-50': !deliveryEnabled.cdek}">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div class="space-y-2">
            ...
            <Input id="cdek-account" v-model="deliveryData.cdek.account" placeholder="Введите логин" :disabled="!deliveryEnabled.cdek" />
          </div>
          <div class="space-y-2">
            <Label for="cdek-secure">Secure password</Label>
            <Input id="cdek-secure" v-model="deliveryData.cdek.securePassword" type="password" placeholder="Введите secure password" :disabled="!deliveryEnabled.cdek" />
          </div>
        </div>
        <div class="space-y-2">
    ...
    <Card>
      <CardHeader>
        <div class="flex items-center space-x-3">
          <div class="w-10 h-10 rounded-md bg-gradient-to-br from-orange-500 to-red-500 flex items-center justify-center text-white">
      ...
      <CardContent class="space-y-4" :class="{'opacity-50': !deliveryEnabled.boxberry}">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div class="space-y-2">
            ...
            <Input id="boxberry-token" v-model="deliveryData.boxberry.token" type="password" placeholder="Введите API токен" :disabled="!deliveryEnabled.boxberry" />
          </div>
          <div class="space-y-2">
            <Label for="boxberry-widget">Код виджета</Label>
            <Input id="boxberry-widget" v-model="deliveryData.boxberry.widgetCode" placeholder="Введите код виджета" :disabled="!deliveryEnabled.boxberry" />
          </div>
        </div>
        <div class="space-y-2">
      ...
      <CardContent class="space-y-4" :class="{'opacity-50': !deliveryEnabled.russianPost}">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div class="space-y-2">
            ...
            <Input id="post-login" v-model="deliveryData.russianPost.login" placeholder="Введите логин" :disabled="!deliveryEnabled.russianPost" />
          </div>
          <div class="space-y-2">
            <Label for="post-password">Пароль</Label>
            <Input id="post-password" v-model="deliveryData.russianPost.password" type="password" placeholder="Введите пароль" :disabled="!deliveryEnabled.russianPost" />
          </div>
        </div>
        <div class="space-y-2">
